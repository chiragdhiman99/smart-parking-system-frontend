      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-16 py-8 flex flex-col lg:flex-row gap-8 items-start">
        ...
        >
          <div className="relative h-48 overflow-hidden">
            <div
              className={`w-full h-full bg-gradient-to-br ${theme} flex items-center justify-center text-7xl`}
            >
              {parking.icon}
            </div>
            <div
        ...
        >
          <div className="flex flex-wrap gap-2 mb-3">
            {parking.vehicleTypes?.map((v) => (
              <span
                key={v}
                className="bg-white/80 backdrop-blur text-gray-700 text-xs font-bold px-3 py-1.5 rounded-xl"
              >
                {v === "2-wheeler" ? "🏍️" : "🚗"} {v}
              </span>
            ))}
            {parking.autoApprove && (
              <span className="bg-green-500/20 text-green-800 text-xs font-bold px-3 py-1.5 rounded-xl">
                ⚡ Instant Confirm
              </span>
            )}
          </div>
          ...
          <p className="text-gray-600 font-medium mb-4">📍 {parking.address}</p>

          <div className="flex items-center gap-3 mb-5">
            <div className="bg-white/80 backdrop-blur px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-700">
